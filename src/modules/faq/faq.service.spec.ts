...
describe('FaqService', () => {
  ...
  it('getFaqById deve retornar FAQ quando existir', async () => {
    ...
    await expect(service.getFaqById(1)).resolves.toEqual(fakeFaq);
  ...
  it('createFaq deve chamar model.create e retornar o FAQ criado', async () => {
    const fakeFaq = { id: 2, pergunta: 'P2', resposta: 'R2' };
    faqModelMock.create.mockResolvedValueOnce(fakeFaq);

    await expect(
      service.createFaq({ pergunta: 'P2', resposta: 'R2' })
    ).resolves.toEqual(fakeFaq);
